...
def translate(message_text, model, src_vocab, trg_vocab, preprocess, postprocess,
    ...
    test_data, src_vocab, trg_vocab = load_line_as_data(
        lowercase=lowercase,
        line=sentence, src_vocab=src_vocab, trg_vocab=trg_vocab,
        level=level
    )
...
def load_model(model_dir, bpe_src_code=None, tokenize=None):
    ...
    conf["src_vocab"] = build_vocab(
        field="src", vocab_file=src_vocab_file,
        dataset=None, max_size=-1, min_freq=0
    )
    conf["trg_vocab"] = build_vocab(
        field="trg", vocab_file=trg_vocab_file,
        dataset=None, max_size=-1, min_freq=0
    )
    ...
    if bpe_src_code is not None and conf["level"] == "bpe":
        # load bpe merge file
